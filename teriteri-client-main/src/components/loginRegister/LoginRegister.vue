...
export default {
    ...
    methods: {
      ...
      async submitLogin() {
        ...
        let result;
        try {
          result = await axios.post("/api/user/account/login", {
            username: this.usernameLogin.toString(),
            password: this.passwordLogin.toString(),
          });
        } catch (e) {
        ...
        // 后续异步操作都加 try-catch，防止单个接口异常影响整体
        try { await this.$store.dispatch("getMsgUnread"); } catch (e) { console.error(e); }
        try { await this.initIMServer(); } catch (e) { console.error(e); }
        try { await this.getFavorites(); } catch (e) { console.error(e); }
        try { await this.getLikeAndDisLikeComment(); } catch (e) { console.error(e); }
