...
//        System.out.println("封装耗时：" + (end - start));
        ...
        if (status == 1 || status == 2) {
            ...
            if (Objects.equals(userId, video.getUid()) || currentUser.isAdmin()) {
                ...
                if (flag > 0) {
                    ...
                    redisUtil.zsetDelMember("user_video_upload:" + video.getUid(), video.getVid());
                    // 从分类索引中移除
                    redisUtil.zsetDelMember("category_video:" + video.getMcId(), video.getVid());
                    if (video.getScId() != null && !video.getScId().isEmpty()) {
                        redisUtil.zsetDelMember("category_video:" + video.getMcId() + ":" + video.getScId(), video.getVid());
                    }
                    // 搞个异步线程去删除OSS的源文件
    ...
    @Override
    public List<Map<String, Object>> getVideosByCategory(String mcId, String scId, Integer rule, Integer page, Integer quantity) {
        try (SqlSession sqlSession = sqlSessionFactory.openSession(ExecutorType.BATCH)) {
            // 构建查询条件
            QueryWrapper<Video> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("mc_id", mcId).eq("status", 1); // 只查询已审核通过的视频
            ...
            // 根据排序规则设置排序
            String sortColumn = "upload_date"; // 默认值
            switch (rule) {
                case 1:
                    ...
                    break;
                case 2:
                    // 按播放量排序需要关联video_stats表
                    queryWrapper.orderByDesc("upload_date"); // 先按时间排序，后面会重新排序
                    break;
                case 3:
                    // 按点赞数排序需要关联video_stats表
                    queryWrapper.orderByDesc("upload_date"); // 先按时间排序，后面会重新排序
                    break;
                default:
                    sortColumn = "upload_date";
            ...
            // 分页查询
            queryWrapper.last("LIMIT " + quantity + " OFFSET " + (page - 1) * quantity);
            List<Video> videos = videoMapper.selectList(queryWrapper);
            ...
            // 如果是按播放量或点赞数排序，需要重新排序
            if (rule == 2 || rule == 3) {
                videos = videos.stream()
                    .sorted((v1, v2) -> {
                        VideoStats stats1 = videoStatsService.getVideoStatsById(v1.getVid());
                        VideoStats stats2 = videoStatsService.getVideoStatsById(v2.getVid());
                        
                        if (rule == 2) { // 按播放量排序
                            return Integer.compare(stats2.getPlay(), stats1.getPlay());
                        } else { // 按点赞数排序
                            return Integer.compare(stats2.getGood(), stats1.getGood());
                        }
                    })
                    .limit(quantity)
                    .collect(Collectors.toList());
            ...
            // 构建返回结果
            List<Map<String, Object>> result = videos.stream().parallel().map(video -> {
                Map<String, Object> map = new HashMap<>();
                map.put("video", video);
    ...
    @Override
    public int getVideoCountByCategory(String mcId, String scId) {
        QueryWrapper<Video> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("mc_id", mcId).eq("status", 1); // 只统计已审核通过的视频
